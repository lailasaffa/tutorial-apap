...
public class RestoranControllerTest {
    ...
    @Test
    public void whenViewRestoranWithRequestParamItShouldShowTheCorrectData() throws Exception{
        RestoranModel dummyRestoran = generateDummyRestoranModel(1);
        when(restoranService.getRestoranByIdRestoran(Long.valueOf(1))).thenReturn(Optional.of(dummyRestoran));
        mockMvc.perform(get("/restoran/view?idRestoran=1"))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.view().name("view-restoran"))
                .andExpect(model().attribute("resto", is(dummyRestoran)));
    }
    @Test
